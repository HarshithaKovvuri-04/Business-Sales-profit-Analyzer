...
def apply_custom_ui():
    ...
    <style>
        ...
        .role-badge {
            ...
            padding: 5px 12px; border-radius: 20px; font-size: 0.85em; font-weight: 600;
        }
    </style>
    """, unsafe_allow_html=True)

# --- 2. DATABASE ENGINE (WITH CONNECTION RECOVERY) ---
@st.cache_resource
def get_engine():
    conn_url = URL.create(
    ...
    return create_engine(
        conn_url, 
        pool_pre_ping=True,  # Vital: Checks connection before every query
        pool_recycle=300     # Refreshes connections every 5 minutes
    )

# Context manager to handle sessions safely
@contextmanager
def get_db_session():
    engine = get_engine()
...
if not st.session_state.user_id:
    ...
    with tab2:
        ...
        with st.form("reg_form"):
            ...
            new_r = st.selectbox("Role", ["Owner", "Manager", "Analyst"])
    ...
    with st.expander("➕ Register a New Business Entity"):
        with st.form("add_biz"):
            ...
            ind = st.selectbox("Industry", ["Retail", "Service", "Food", "Manufacturing"])
